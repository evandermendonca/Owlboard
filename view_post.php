...
$dot_id = $dot_id + 1;
  	if($result->num_rows == 1)
  	{
		...
		if($row['post_category']=="Complaint") {
		   ...
		   $comment_query ="SELECT * FROM Comments WHERE post_id ='$post_id' ORDER BY time_posted DESC";
		   $special_action = "";
		   if($role =="1")
		   {
	  ...
	  <div class="comments">
	  	<div class="post-comment">
			<form action="php/comment.php" method="post">
				...
				<br>
				<button id="comment-button" type="submit" name="s_btn">Post Comment</button>
			</form>
		</div>
EOL;
...
$conn -> close();
  ...
  or die("Connect failed: %s\n". $conn1 -> error);
   		 ...
   		 if($comment_result = $conn1->query($comment_query))
   		 {
   		 	if($comment_result->num_rows > 0)
   		 	{
   		 		while($single_comment=$comment_result->fetch_array())
   		 		{

  		$comment_owner = $single_comment['comment_owner'];
  		$comment_text = $single_comment['comment_text'];
		$time_posted = $single_comment['time_posted'];
		$time_posted2 = date("m/d/y - g:i A", strtotime("$time_posted"));

   echo <<<EOL
		<div class="individual-comment-container">
			<div class="comment-content">
				<div class="comment-info-section">
					<span>$comment_owner</span>
					<p>$comment_text</p>
				</div>
				<div class="comment-time-section">
					<span>$time_posted2</span>
				</div>
			</div>
			<hr>
		</div>
...
EOL;
			...
			echo <<<EOL
						...
						<div class="error-message">
							<span>Error loading the comment section. Try reloading the page</span>
						</div>
					</div>
				</div>
			</div>
